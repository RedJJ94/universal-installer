...
if (hasFirebaseConfig) {
    ...
    extensions.findByName("googleServices")?.let { ext ->
        try {
            ...
            val ignoreStrategy =
                strategyClass.enumConstants?.firstOrNull {
                    (it as Enum<*>).name == "IGNORE"
                }
            if (ignoreStrategy != null) {
                ext.javaClass
                    .getMethod(
                        "setMissingGoogleServicesStrategy",
                        strategyClass
                    )
...
android {
    ...
    // Analytics/Crashlytics libraries out of the build we publish as open source.
...
dependencies {
     ...
     * Ackpine
     *
     * Ackpine's Shizuku backend is retained because Porter exposes the same
     * Shizuku-compatible API surface. Do not remove ackpine-shizuku.
     *
     * Porter supplies the Shizuku-compatible API/AIDL/shared modules, so the
     * upstream dev.rikka.shizuku modules must not be selected alongside Porter.
     */
    ...
    /*
     * Porter capability resolution.
     *
     * Porter intentionally advertises the Shizuku capabilities so libraries
     * such as Ackpine can use Porter's maintained Shizuku-compatible API.
     *
     * When Gradle sees both the original Shizuku module and Porter's compatible
     * module, explicitly select Porter's implementation.
     */
    configurations.configureEach {
        resolutionStrategy.capabilitiesResolution {
            withCapability("dev.rikka.shizuku:api") {
                select("com.github.d4rken-org.porter-api:api:0.1.0")
            }

            withCapability("dev.rikka.shizuku:aidl") {
                select("com.github.d4rken-org.porter-api:aidl:0.1.0")
            }

            withCapability("dev.rikka.shizuku:shared") {
                select("com.github.d4rken-org.porter-api:shared:0.1.0")
            }
        }
    }
    ...
    // Porter privileged-service backend.
    ...
    // App Updater module — opensource flavor only.
    ...
    // Play In-App Review, `play` flavor only.
    ...
    // Google Code Scanner (ML Kit) for instant QR scanning (`play` only).
    "playImplementation"(libs.play.services.code.scanner)

    // Wear OS Data Layer client (`play` only).
